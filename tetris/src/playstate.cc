...
void PlayState::initDobestUpdate(GameEngine* game) {
    if(game->mode == 1)
    {
        // mouse area setting
...
// Restarts game.
void PlayState::reset(GameEngine* game) {
    // Empty the board.
    ...
    for (int i = 0; i < board->ROWS; i++)
        for (int j = 0; j < board->COLS; j++)
            if (board->color[i][j] != -1) {
                ...
                if((game->mode == 1)&&(board->color[i][j] == 10)) {
                    SDL_Rect block_dst;
